...
/*
 * GR Template Consolidation Script
 * Requires: Enable Advanced Google Service "Drive API"
 */
...
const HEADER_ALIASES = {
  "Acceptance Date (PAC/FAC)": [
    "acceptance date (pac/fac)", "acceptance date", "pac/fac",
    "pac approved date", "fac approved date", "approved date"
  ],
  "PO No.": ["po no.", "po no", "po number", "purchase order no"],
  "PO Item No.": ["po item no.", "po item no", "po item number"],
  "PO Service Item No.": ["po service item no.", "po service item no", "service item no"],
...
// Keywords in row cells that indicate a summary/footer row to be skipped
const SUMMARY_KEYWORDS = [
  "grand total", "sub-total", "subtotal",
  "p.o amount", "po amount", "final amount", "amount due",
  "prepared by", "checked by", "approved by", "noted by",
  "signature", "printed name", "remarks:"
];
...
function setupAutomaticEvery1Min() {
  ...
  ScriptApp.getProjectTriggers().forEach(function(t) {
    if (t.getHandlerFunction() === "consolidateGRTemplateData") ScriptApp.deleteTrigger(t);
  });
...
function consolidateGRTemplateData() {
  ...
  try {
    ...
    if (candidates["2025"].length === 0 && candidates["2026"].length === 0) {
      Logger.log("No new files to process. Trigger is still active and checks again next minute.");
      return;
    }
...
function ensureSheets_(ss) {
  ...
  ["2025", "2026"].forEach(function(year) {
    ...
    if (sh.getLastRow() === 0) sh.appendRow(COLUMN_MAPPING.concat([CONFIG.sourceHeaderName]));
  });
...
function loadFailedAttemptsMap_(ss) {
  ...
  data.forEach(function(row) {
    ...
    const status = String(row[4] || "").trim().toLowerCase();
    if (!fileName) return;
    if (status === "done" || status === "ok") { map[fileName] = 0; return; }
    if (status.indexOf("no data extracted") === 0 || status.indexOf("error:") === 0) {
...
function listCandidateFilesByYear_(sourceFolder, processedMap, failedAttemptsMap) {
  ...
  while (files.hasNext()) {
    ...
    if (!year) continue;
    candidates[year].push({ id: f.getId(), name: name, year: year, url: f.getUrl() });
  }
  return candidates;
}
...
function processSingleFile_(ss, fileInfo, tempFolder) {
  ...
  try {
    ...
    if (rows.length === 0) return { rowsAdded: 0, status: "No data extracted" };
    ...
    return { rowsAdded: 0, status: "Error: " + e.message };
  } finally {
    if (tempFileId) {
...
function openSpreadsheetWithRetry_(fileId, attempts, delayMs) {
  ...
  for (var i = 1; i <= attempts; i++) {
    ...
    catch (e) { lastErr = e; Utilities.sleep(delayMs * i); }
  }
...
function parseConvertedSheet_(tempSS, fileName) {
  ...
  for (let i = 0; i < sheets.length; i++) {
    ...
    if (h.headerRowIndex < 0 || h.matchedCount < CONFIG.minHeaderMatches) continue;
    ...
    if (!best || rows.length > best.rows.length ||
        (rows.length === best.rows.length && h.matchedCount > best.h.matchedCount)) {
      best = { rows: rows, h: h, sheetName: sh.getName() };
    }
  }
...
function detectHeaderRowAndMap_(values) {
  ...
  for (let r = 0; r < scanRows; r++) {
    ...
    if (result.count > bestCount) {
      bestCount = result.count; bestRow = r; bestMap = result.map;
    }
...
// Detects "Total:", "Total", "Grand Total" and other footer/summary rows
function isSummaryOrFooterRow_(rawRow, dispRow) {
  ...
  for (let i = 0; i < rawRow.length; i++) {
    ...
    // "total" or "total:" appearing alone in any cell = summary row
    if (cell === "total" || cell === "total:") return true;

    // Broader keyword check limited to first 8 cells
    if (i < 8) {
...
// Detects rows where the same non-empty value repeats 4+ times (merged-cell artifact)
function isRepeatedValueRow_(rawRow, dispRow) {
  const disp = dispRow || [];
  const freq = {};
  let nonEmpty = 0;
...
function extractRowsWithFilter_(values, display, columnMap, headerRowIndex, strict) {
  const rows = [];
  // Only apply the output-anchor check if PO No. or Material Description was detected
  ...
  for (let r = headerRowIndex + 1; r < values.length; r++) {
    ...
    if (isRepeatedValueRow_(rawRow, dispRow)) continue;
    ...
    // Skip rows where PO No., Material Description, and GR Mat. Doc. are all blank
    // These are typically summary/amount-only rows from different sections of the template
    if (checkOutputAnchors) {
      const poVal = String(outRow[1] || "").trim();         // PO No.
      const matVal = String(outRow[4] || "").trim();        // Material Description
      const grDocVal = String(outRow[9] || "").trim();      // GR Mat. Doc.
...
function extractRowsByAnchors_(values, display, columnMap, headerRowIndex) {
  ...
  for (let r = headerRowIndex + 1; r < values.length; r++) {
    ...
    // Same output anchor check as extractRowsWithFilter_
...
function logToTracker_(ss, fileInfo, result) {
  ...
  if (!tracker) return;
  tracker.appendRow([
    new Date(), fileInfo.name, fileInfo.year, result.rowsAdded, result.status,
    '=HYPERLINK("' + fileInfo.url + '","' + fileInfo.name.replace(/"/g, '""') + '")'
  ]);
}
...
function requeueNoDataFiles() {
  ...
  for (let i = data.length - 1; i >= 0; i--) {
    ...
    if (status !== "ok" && status !== "done") { tracker.deleteRow(i + 2); removed++; }
  }
...
function debugTestSingleFile() {
  const sourceFolder = findFolder_(CONFIG.sourceFolderName);
  if (!sourceFolder) { SpreadsheetApp.getUi().alert("Source folder not found."); return; }
  ...
  while (files.hasNext()) {
    ...
    if ((n.indexOf("2025") !== -1 || n.indexOf("2026") !== -1) &&
        (n.indexOf(".xlsx") !== -1 || n.indexOf(".xls") !== -1)) { testFile = f; break; }
  }
  if (!testFile) { SpreadsheetApp.getUi().alert("No 2025/2026 Excel file found."); return; }
