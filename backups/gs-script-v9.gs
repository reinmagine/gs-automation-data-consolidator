...
function loadPlaLookupMap_(ss) {
  ...
  data.forEach(function (row) {
    const key = normalizeLookupKey_(row[colPla - 1]);
    if (!key) return;
...
function repairMainSiteColumnsNow() {
  ...
  ["2025", "2026"].forEach(function (year) {
    ...
    if (plaCol < 1 || matCol < 1 || svcCol < 1 || amtCol < 1 || curCol < 1) {
      details.push(year + ": one or more required headers not found");
      return;
    }
    ...
    for (var i = 0; i < rowCount; i++) {
      const key = normalizeLookupKey_(plaVals[i][0]);
      ...
      if (!territory) {
        territory = isManagedServices_(matVals[i][0], svcVals[i][0])
          ? "Managed Services"
          : "N/A";
      }
...
function appendRowsWithSourceLink_(sheet, rows, fileInfo) {
  ...
  const enrich = rows.map(function (r) {
    ...
    const key = normalizeLookupKey_(r[14]);
    const found = lookupMap[key];
    ...
    if (!territory) {
      territory = isManagedServices_(r[4], r[5]) ? "Managed Services" : "N/A";
    }
...
function convertAmountToUsdForAllData() {
  ...
  ["2025", "2026"].forEach(function (year) {
    ...
    const usdOut = [];
    for (var i = 0; i < rowCount; i++) {
      usdOut.push([toUsdIfPhp_(amtVals[i][0], curVals[i][0])]);
    }
    ...
    total += rowCount;
    notes.push(year + ": USD recomputed for " + rowCount + " row(s)");
  });
