...
function loadAllTrackerData_(ss) {
  ...
  const displayValues = tracker.getRange(2, 1, rowCount, maxCol).getDisplayValues();
  ...
  for (let i = 0; i < rowCount; i++) {
    ...
    if (isDoneStatus_(status)) {
      ...
      result.failedAttemptsMap[fileName] = (result.failedAttemptsMap[fileName] || 0) + 1;
    }
  }
...
function getEnrichmentForRow_(row, lookupMap) {
  ...
  var installedKey = normalizeInstalledPlaLookupKey_(row[COL["Installed PLA ID"]]);
  ...
  if (isMissingOrNaTerritory_(territory)) {
    if (isOpexWbs_(wbsVal)) {
      territory = "OPEX";
    } else if (isManagedServices_(row[COL["Material Description"]], row[COL["PO Service Short Text"]])) {
...
function normalizePlaLookupKey_(v) {
  var s = String(v || "").toUpperCase().replace(/\s+/g, "").trim();
  if (!s) return "";
...
function normalizeInstalledPlaLookupKey_(v) {
  ...
  if (/^\s*(n\/?a|na|#name\?|not mentioned on po|no pla id|no pla|various pla|various site|search ring|searchring|multiple core)\b/i.test(low)) return "";
  ...
  token = String(token || "").toUpperCase().replace(/\s+/g, "").trim();
...
function isManagedServices_(materialDesc, serviceShortText) {
  ...
  return (a.indexOf("managed services") !== -1 || b.indexOf("managed services") !== -1);
}

function normalizeCurrencyCode_(currencyValue) {
  var c = String(currencyValue || "").replace(/\u00a0/g, " ").toLowerCase().trim();
  if (!c) return "";

  if (c.indexOf("php") !== -1 || c.indexOf("peso") !== -1 || c.indexOf("philippine peso") !== -1 || c.indexOf("ph peso") !== -1 || c.indexOf("₱") !== -1) {
    return "PHP";
  }
  if (c.indexOf("usd") !== -1 || c.indexOf("us dollar") !== -1 || c.indexOf("us dollars") !== -1 || c.indexOf("$") !== -1) {
    return "USD";
  }
...
function isPhpCurrency_(currencyValue) { return normalizeCurrencyCode_(currencyValue) === "PHP"; }
function isUsdCurrency_(currencyValue) { return normalizeCurrencyCode_(currencyValue) === "USD"; }
function isEurCurrency_(currencyValue) { return normalizeCurrencyCode_(currencyValue) === "EUR"; }
...
function parseAmount_(v) {
  ...
  normalized = normalized.replace(/us\s*dollars?/g, "").replace(/philippine\s*pesos?/g, "").replace(/pesos?/g, "").replace(/euros?/g, "").replace(/usd/g, "").replace(/php/g, "").replace(/eur/g, "").replace(/[\s,.$₱€()\-]/g, "");
  if (/[^0-9]/.test(normalized)) return NaN;

  s = s.replace(/\s+/g, "").replace(/,/g, "").replace(/\$/g, "").replace(/₱/g, "").replace(/€/g, "").replace(/usd/gi, "").replace(/php/gi, "").replace(/eur/gi, "").replace(/pesos?/gi, "").replace(/euros?/gi, "").replace(/us\s*dollars?/gi, "").replace(/philippine\s*pesos?/gi, "");
  s = s.replace(/[^0-9.-]/g, "");
...
function loadPlaLookupMap_(ss) {
  ...
  function findHeaderIndex_(candidates) {
    for (var i = 0; i < headers.length; i++) {
      var current = String(headers[i] || "").trim().toLowerCase();
      for (var j = 0; j < candidates.length; j++) {
        if (current === String(candidates[j] || "").trim().toLowerCase()) {
          return i;
        }
      }
    }
  ...
  if (colPla < 1 || colReg < 1 || colSite < 1 || colTerr < 1) {
    throw new Error("PLA Lookup headers missing. Required: PLA ID, SITE NAME, REGION (or Regional Area), TERRITORY");
  }
...
function getSpreadsheet_() {
  try { const active = SpreadsheetApp.getActiveSpreadsheet(); if (active) return active; } catch (e) { }
  const id = PropertiesService.getScriptProperties().getProperty("BOUND_SPREADSHEET_ID");
  if (!id) return null;
  try { return SpreadsheetApp.openById(id); } catch (e2) { return null; }
}
...
function ensureSheets_(ss) {
  ...
  years.forEach(function (year) {
    try {
      ...
      if (!sh) return;
      if (sh.getLastRow() === 0) {
        sh.appendRow(COLUMN_MAPPING.concat([CONFIG.sourceHeaderName]).concat(CONFIG.enrichmentHeaders));
      } else {
  ...
  if (tracker.getLastRow() === 0) {
    tracker.appendRow(["Timestamp", "File Name", "Month", "Year", "Rows Added", "Status", "File Link"]);
  }

  const lookup = ss.getSheetByName(CONFIG.lookupSheetName);
  if (lookup.getLastRow() === 0) {
    lookup.appendRow(["PLA ID", "SITE NAME", "FORMATTED ADDRESS", "REGION", "TERRITORY"]);
  }
}
...
function ensureEnrichmentColumns_(sheet) {
  ...
  return { regionalAreaCol: baseCol, cleanedSiteNameCol: baseCol + 1, territoryCol: baseCol + 2, usdCol: baseCol + 3 };
}
...
function isOpexWbs_(wbsValue) {
  var w = String(wbsValue || "").toUpperCase().replace(/\s+/g, "").trim();
  if (!w) return false;
...
function getOutputSheetNameForYear(year) {
  ...
  if (CONFIG.outputSheets && CONFIG.outputSheets[year]) return CONFIG.outputSheets[year];
...
function normalizeFileKey_(name) {
  return String(name || "").normalize("NFKC").toLowerCase().replace(/\.[a-z0-9]{2,5}$/i, "").replace(/[^a-z0-9]+/g, "").trim();
}
...
function normalizeStatus_(statusText) { return String(statusText || "").trim().toLowerCase(); }
function isFailureStatus_(statusText) {
  const s = normalizeStatus_(statusText);
  return (s.indexOf("no data extracted") === 0 || s.indexOf("error:") === 0 || s.indexOf("needs manual check") === 0);
}
...
function normalizeUrlAltKey_(url) { return String(url || "").replace(/^https?:\/\//i, "").replace(/\?.*$/, "").trim().toLowerCase(); }
...
function buildTrackerExactDoneKeyMap_(ss) {
  ...
  const fileNames = tracker.getRange(2, fileNameCol, rowCount, 1).getDisplayValues();
  const statuses = tracker.getRange(2, statusCol, rowCount, 1).getDisplayValues();
...
function trackerHasProcessedEntry_(ss, fileInfo) {
  ...
  const targetKeyMap = {};
  buildFileProcessingKeys_(fileInfo).forEach(function (k) { if (k) targetKeyMap[k] = true; });
...
function repairMainSiteColumnsNow() {
  ...
  years.forEach(function (year) {
    ...
    const wbsVals = wbsCol > 0 ? sh.getRange(2, wbsCol, rowCount, 1).getDisplayValues() : [];

    const plaVals = plaCol > 0 ? sh.getRange(2, plaCol, rowCount, 1).getDisplayValues() : [];
    const matVals = matCol > 0 ? sh.getRange(2, matCol, rowCount, 1).getDisplayValues() : [];
    const svcVals = svcCol > 0 ? sh.getRange(2, svcCol, rowCount, 1).getDisplayValues() : [];
    const poUnitVals = poUnitCol > 0 ? sh.getRange(2, poUnitCol, rowCount, 1).getDisplayValues() : [];
    const subVals = subCol > 0 ? sh.getRange(2, subCol, rowCount, 1).getDisplayValues() : [];
  ...
  notify_("Lookup + USD update complete.\nTotal rows updated: " + totalUpdated + "\n\n" + details.join("\n"));
}
...
function appendRowsWithSourceLink_(sheet, rows, fileInfo, controllerSs) {
  ...
  if (sheet.getLastRow() === 0) {
    sheet.appendRow(COLUMN_MAPPING.concat([CONFIG.sourceHeaderName]).concat(CONFIG.enrichmentHeaders));
  }
  ...
  const sourceCol = getColumnIndexByHeader_(sheet, CONFIG.sourceHeaderName);
  if (sourceCol > 0) {
    const linkFormula = '=HYPERLINK("' + fileInfo.url + '","' + fileInfo.name.replace(/"/g, '""') + '")';
    const linkFormulas = Array.from({ length: rows.length }, function () { return [linkFormula]; });
    sheet.getRange(startRow, sourceCol, rows.length, 1).setFormulas(linkFormulas);
  }
  ...
  sheet.getRange(startRow, colInfo.regionalAreaCol, rows.length, 4).setValues(enrich);
}
...
function convertAmountToUsdForAllData() {
  ...
  getConfiguredYears_().forEach(function (year) {
    ...
    const subVals = subCol > 0 ? sh.getRange(2, subCol, rowCount, 1).getDisplayValues() : [];
    const curVals = sh.getRange(2, curCol, rowCount, 1).getDisplayValues();
    const poVals = poUnitCol > 0 ? sh.getRange(2, poUnitCol, rowCount, 1).getDisplayValues() : [];
    const colInfo = ensureEnrichmentColumns_(sh);
    ...
    if (repairedAmountCount > 0) sh.getRange(2, amtCol, rowCount, 1).setValues(amtOut);
    if (repairedCurrencyCount > 0) sh.getRange(2, curCol, rowCount, 1).setValues(curOut);
    ...
    total += rowCount;
    notes.push(year + ": USD recomputed for " + rowCount + " row(s), Amount repaired: " + repairedAmountCount);
  });

  notify_("USD conversion done (rate = 57).\nTotal rows processed: " + total + "\n\n" + notes.join("\n"));
}
...
function fixSourceFileHyperlinksNow() {
  ...
  getConfiguredYears_().forEach(function (year) {
    ...
    const fileMaps = sourceFolder ? buildFileUrlMapByName_(sourceFolder) : { byName: {}, byNorm: {}, filesArr: [] };
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      if (cellValue) {
        if (urlByName[cellValue]) {
          ...
          if (nk && urlByNorm[nk] && urlByNorm[nk].length === 1) {
            ...
            if (m && m[1]) {
              const token = m[1];
              const found = filesArr.filter(function (f) { return f.nameLower.indexOf(token) !== -1; });
      ...
      if (matchedUrl) {
        const linkFormula = '=HYPERLINK("' + matchedUrl.replace(/"/g, '""') + '","' + cellValue.replace(/"/g, '""') + '")';
    ...
    if (fixed > 0) {
      ...
      totalFixed += fixed;
      details.push(year + ": converted " + fixed + " filename(s) to hyperlink formula(s)");
    } else {
  ...
  let msg = "Source File hyperlink conversion complete.\nTotal converted: " + totalFixed + "\n\n" + details.join("\n");
  if (backfillMsg) msg += "\n\n" + backfillMsg;
  notify_(msg);
}
...
function appendTrackerRowIfNotDuplicate_(ss, fileInfo, result, processedMap) {
  try {
    ...
    tracker.appendRow([new Date(), fileInfo.name, month, fileInfo.year, result.rowsAdded, result.status, sourceUrl]);
...
function ensurePerfSheetExists_() {
  ...
  if (!sh) {
    sh = ss.insertSheet(name);
    sh.appendRow(["Timestamp", "File Name", "Year", "ConvertMs", "OpenMs", "ParseMs", "AppendMs", "TotalMs", "RowsAdded", "Status"]);
  }
  return sh;
}
...
function logPerfEntry_(fileInfo, timings) {
  try {
    ...
    if (!sh) return;
    sh.appendRow([new Date(), fileInfo.name, fileInfo.year, timings.convertMs || 0, timings.openMs || 0, timings.parseMs || 0, timings.appendMs || 0, timings.totalMs || 0, timings.rowsAdded || 0, timings.status || ""]);
  } catch (e) {
...
function normalizeTrackerFileLinkColumnNow() {
  ...
  notify_("Tracker File Link normalization complete.\nRows scanned: " + rowCount + "\nRows updated: " + changed + "\nRecovered by file name match: " + recoveredByName);
}
...
function buildFileUrlMapByName_(folder) {
  ...
  while (files.hasNext()) {
    ...
    const url = f.getUrl();
    const id = f.getId();
    filesArr.push({ name: name, nameLower: String(name).toLowerCase(), url: url, id: id });
...
function backfillMissingSourceLinks_(dryRun, options) {
  ...
  const sourceFolder = findFolder_(CONFIG.sourceFolderName);
  if (!sourceFolder) return "Source folder not found: " + CONFIG.sourceFolderName;

  const filesArr = [];
  const files = sourceFolder.getFiles();
  while (files.hasNext()) {
    const f = files.next();
    filesArr.push({ name: String(f.getName() || ""), nameLower: String(f.getName() || "").toLowerCase(), url: f.getUrl(), id: f.getId() });
  }

  const years = getConfiguredYears_();
  let totalScanned = 0, totalUpdated = 0, totalAmbiguous = 0, totalNoMatch = 0;
  const notes = [];

  years.forEach(function (year) {
    const sh = findOutputSheetByYear_(ss, year);
    if (!sh) { notes.push(year + ": sheet not found"); return; }

    const lastRow = sh.getLastRow();
    if (lastRow < 2) { notes.push(year + ": no data rows"); return; }

    const sourceCol = getColumnIndexByHeader_(sh, CONFIG.sourceHeaderName);
    if (sourceCol < 1) { notes.push(year + ": Source File column not found"); return; }
    ...
    const pendingWrites = [];
    let scanned = 0, updated = 0, ambiguous = 0, nomatch = 0;
    let runStart = null;
    let runFormulas = [];
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      scanned++;
      if (existingFormula && existingFormula.indexOf("=") === 0) { flushRun(); continue; }
      if (existingDisplay) { flushRun(); continue; }
      ...
      const tokensClean = tokens.filter(function (t) { return t && String(t).trim() !== ""; }).map(function (t) { return String(t).toLowerCase(); });

      if (tokensClean.length === 0) { flushRun(); nomatch++; continue; }
      ...
      if (candidates.length === 1) {
        ...
        formulaToWrite = '=HYPERLINK("' + f.url.replace(/"/g, '""') + '","' + f.name.replace(/"/g, '""') + '")';
        updated++;
      } else if (candidates.length > 1) {
        candidates.sort(function (a, b) { return b.matchCount - a.matchCount; });
    ...
    if (!dryRun && pendingWrites.length > 0) {
      for (let w = 0; w < pendingWrites.length; w++) {
        ...
        try {
          sh.getRange(2 + write.startRow, sourceCol, write.formulas.length, 1).setFormulas(write.formulas);
        } catch (e) {
          notes.push(year + ": error writing Source File column (" + e.message + ")");
          break;
        }
      }
    }
    ...
    notes.push(year + ": scanned=" + scanned + ", updated=" + updated + ", ambiguous=" + ambiguous + ", no match=" + nomatch + (dryRun ? " (dry run)" : ""));
  });

  const msg = "Backfill complete. Scanned: " + totalScanned + ", updated: " + totalUpdated + ", ambiguous: " + totalAmbiguous + ", no match: " + totalNoMatch + "\n\n" + notes.join("\n");
  if (!dryRun && !options.suppressNotify) notify_(msg);
  return msg;
}

function backfillMissingSourceLinksPreview() { return backfillMissingSourceLinks_(true); }
function backfillMissingSourceLinksNow() { return backfillMissingSourceLinks_(false); }

function exportBackfillAmbiguousExamplesNow() {
  const ss = getSpreadsheet_();
  if (!ss) { notify_("No spreadsheet found."); return; }

  const sourceFolder = findFolder_(CONFIG.sourceFolderName);
  if (!sourceFolder) { notify_("Source folder not found: " + CONFIG.sourceFolderName); return; }
  ...
  if (diagSh) { diagSh.clear(); } else { diagSh = ss.insertSheet(diagSheetName); }

  const header = ["Year", "Sheet", "Row", "PO No.", "GR Mat. Doc.", "WBS Element", "PO PLA ID", "Installed PLA ID", "Tokens", "CandidateCount", "TopCandidates"];
  ...
  years.forEach(function (year) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (candidates.length > 1) {
        ...
        const topWins = top.matchCount > (second ? second.matchCount : 0);
        if (!topWins && !pickTopOnTie) {
          const topList = candidates.slice(0, 5).map(function (c) { return c.matchCount + ":" + c.file.name + " (" + c.file.id + ")"; }).join(" | ");
          outRows.push([year, sh.getName(), 2 + i, poCol > 0 ? row[poCol - 1] : "", grCol > 0 ? row[grCol - 1] : "", wbsCol > 0 ? row[wbsCol - 1] : "", poPlaCol > 0 ? row[poPlaCol - 1] : "", instPlaCol > 0 ? row[instPlaCol - 1] : "", tokensClean.join(" | "), candidates.length, topList]);
        }
      }
    }
  });
...
function backfillTrackerMonthsNow() {
  ...
  const tracker = ss.getSheetByName(CONFIG.trackerSheetName);
  if (!tracker || tracker.getLastRow() < 2) { notify_("No tracker rows found."); return; }

  const fileNameCol = getColumnIndexByHeader_(tracker, "File Name");
  const monthCol = getColumnIndexByHeader_(tracker, "Month");

  if (fileNameCol < 1 || monthCol < 1) { notify_("File Name or Month column not found in tracker."); return; }

  const sourceFolder = findFolder_(CONFIG.sourceFolderName);
  if (!sourceFolder) { notify_("Source folder not found."); return; }
  ...
  for (let i = 0; i < fileNames.length; i++) {
    ...
    if (fileName && fileMap[fileName]) {
      try {
        ...
        found++;
      } catch (e) { Logger.log("Could not get date for file: " + fileName); }
    }
    months.push([month]);
  }

  tracker.getRange(2, monthCol, rowCount, 1).setValues(months);
  notify_("Month backfill complete. Updated " + rowCount + " row(s), matched " + found + " file(s).");
}
...
function requeueNoDataFiles() {
  ...
  let statusCol = getColumnIndexByHeader_(tracker, "Status");
  if (statusCol < 1) statusCol = 6;
  const data = tracker.getRange(2, statusCol, tracker.getLastRow() - 1, 1).getDisplayValues();
  let removed = 0;
  ...
  if (ss2) ss2.toast("Requeued " + removed + " file(s) for retry.", "GR Automation", 8);
}
...
function cleanupDuplicatesPreview() { return cleanupDuplicates_(true); }

function cleanupDuplicatesNow() {
  try {
    const ui = SpreadsheetApp.getUi();
    const r = ui.alert("Cleanup Duplicates", "This will remove duplicate rows from the tracker and output sheets. Continue?", ui.ButtonSet.YES_NO);
    if (r !== ui.Button.YES) {
...
function cleanupDuplicates_(dryRun) {
  ...
  const docLock = typeof LockService.getDocumentLock === "function" ? LockService.getDocumentLock() : null;

  const gotScriptLock = lock.tryLock(5000);
  const gotDocLock = !docLock || docLock.tryLock(5000);
  if (!gotScriptLock || !gotDocLock) {
    try { if (gotDocLock && docLock) docLock.releaseLock(); } catch (e) { }
    try { if (gotScriptLock) lock.releaseLock(); } catch (e) { }
    const msg = "Another run is active. Please retry cleanup after current processing completes.";
    notify_(msg);
    return msg;
  }
  ...
  try {
    const notes = [];
    let trackerPlan = { duplicateGroupCount: 0, deleteRows: [], previewExamples: [], duplicateRuns: [] };
    ...
    const outputPlans = buildOutputDuplicateCleanupPlansFromTracker_(ss, trackerPlan);
    ...
    notes.push("Tracker duplicates: groups=" + trackerPlan.duplicateGroupCount + ", rows " + (dryRun ? "to remove=" : "removed=") + (dryRun ? trackerPlan.deleteRows.length : removedTracker));

    const trackerRunsWithRows = (trackerPlan.duplicateRuns || []).filter(function (run) { return Number(run.rowsAdded || 0) > 0; }).length;
    notes.push("Tracker duplicate run entries with Rows Added > 0: " + trackerRunsWithRows + ".");

    if (dryRun && trackerPlan.previewExamples.length > 0) {
      notes.push("Tracker examples (keep row -> remove rows):");
      trackerPlan.previewExamples.forEach(function (line) { notes.push("  " + line); });
    }

    notes.push("Output duplicates: groups=" + outputGroupCount + ", rows " + (dryRun ? "to remove=" : "removed=") + (dryRun ? outputDeleteCount : removedOutput) + " (safe match: source + row signature)");

    outputPlans.forEach(function (plan) {
      notes.push(plan.sheet.getName() + ": groups=" + plan.duplicateGroupCount + ", rows " + (dryRun ? "to remove=" : "removed=") + (dryRun ? plan.deleteRows.length : plan.deletedCount || 0));

      if (dryRun && plan.previewExamples && plan.previewExamples.length > 0) {
        notes.push(plan.sheet.getName() + " examples (keep row -> remove rows):");
        plan.previewExamples.forEach(function (line) { notes.push("  " + line); });
      }

      if (plan.warnings && plan.warnings.length > 0) {
        notes.push(plan.sheet.getName() + " warnings:");
        plan.warnings.forEach(function (line) { notes.push("  " + line); });
      }
    });

    if (!dryRun) {
      notes.push("Total rows removed: " + String(removedTracker + removedOutput) + ".");
    } else {
      notes.push("Total rows to remove: " + String(trackerPlan.deleteRows.length + outputDeleteCount) + ".");
    }

    const header = dryRun ? "Duplicate cleanup preview complete." : "Duplicate cleanup complete.";
    ...
    try { if (docLock) docLock.releaseLock(); } catch (e) { }
...
function buildTrackerDuplicateCleanupPlan_(tracker) {
  ...
  for (let i = 0; i < rowCount; i++) {
    ...
    const rowsAdded = toNonNegativeInt_(values[i][rowsAddedCol - 1], display[i][rowsAddedCol - 1]);
    if (!groups[key]) groups[key] = { doneRows: [] };
    groups[key].doneRows.push({ rowNum: rowNum, rowDisplay: display[i], tsMs: tsMs, fileName: fileName, fileUrl: linkUrl, year: year, rowsAdded: rowsAdded });
  }
  ...
  Object.keys(groups).forEach(function (k) {
    ...
    for (let i = 1; i < doneRows.length; i++) {
      ...
      if (shouldKeepNewTrackerRow_(keep.rowNum, keep.tsMs, row.rowNum, row.tsMs)) {
        keep = row;
      }
    }
    ...
    doneRows.forEach(function (row) {
      ...
      duplicates.push({ rowNum: row.rowNum, rowDisplay: row.rowDisplay, rowsAdded: row.rowsAdded });
      duplicateRuns.push({ duplicateKey: k, trackerRowNum: row.rowNum, keepTrackerRowNum: keep.rowNum, fileName: row.fileName, fileUrl: row.fileUrl, year: String(row.year || "").trim(), rowsAdded: row.rowsAdded, tsMs: row.tsMs });
    });
  ...
  return { duplicateGroupCount: duplicateGroupCount, deleteRows: deleteRows, previewExamples: previewExamples, duplicateRuns: duplicateRuns };
}

function buildOutputDuplicateCleanupPlansFromTracker_(ss, trackerPlan) {
  const runs = trackerPlan && trackerPlan.duplicateRuns ? trackerPlan.duplicateRuns : [];
  ...
  runs.forEach(function (run) {
    ...
    if (!groupedBySource[groupKey]) {
      groupedBySource[groupKey] = { year: year, sourceKey: sourceKey, sourceText: String(run.fileName || "").trim(), sourceUrl: String(run.fileUrl || "").trim(), rowsToDelete: 0 };
    }
  ...
  Object.keys(groupedBySource).forEach(function (groupKey) {
    ...
    const sourcePlan = buildOutputDuplicateRowsForSource_(sh, g.sourceText, g.sourceUrl, g.rowsToDelete);
    const sheetName = sh.getName();
    if (!plansBySheet[sheetName]) {
      plansBySheet[sheetName] = { year: g.year, sheet: sh, duplicateGroupCount: 0, deleteRows: [], previewExamples: [], warnings: [] };
    }
    ...
    if (sourcePlan.deleteRows.length > 0) {
      ...
      if (sourcePlan.previewExample) { plan.previewExamples.push(sourcePlan.previewExample); }
    }
  ...
  return Object.keys(plansBySheet).map(function (sheetName) {
    ...
    plan.deleteRows = Object.keys(uniqueRows).map(function (k) { return Number(k); }).sort(function (a, b) { return a - b; });
    return plan;
  });
}

function buildOutputDuplicateRowsForSource_(sheet, sourceText, sourceUrl, rowsToDeleteTarget) {
  ...
  const sourceCol = getColumnIndexByHeader_(sheet, CONFIG.sourceHeaderName);
  if (sourceCol < 1) {
    result.warning = "Source File column not found for " + sheet.getName() + ".";
    return result;
  }
  ...
  for (let i = 0; i < rowCount; i++) {
    ...
    const sourceCellUrl = extractUrlFromCell_(sourceCellText, formulas[i][sourceCol - 1]);
    const sourceKey = buildSourceDuplicateKey_(sourceCellText, sourceCellUrl);
    if (sourceKey !== targetKey) continue;

    matches.push({ rowNum: i + 2, signature: buildOutputRowSignature_(display[i]) });
  }
  ...
  Object.keys(signatureRows).forEach(function (sig) {
    signatureRows[sig].sort(function (a, b) { return a - b; });
  });

  const latestFirst = matches.slice().sort(function (a, b) { return b.rowNum - a.rowNum; });
  ...
  if (deleteRows.length === 0) {
    result.warning = "No safe duplicate rows found for key " + shortenPreviewKey_(targetKey, 48) + ".";
    return result;
  }

  const sortedDeleteRows = deleteRows.slice().sort(function (a, b) { return a - b; });
  ...
  const keepCandidates = matches.map(function (m) { return m.rowNum; }).filter(function (n) { return sortedDeleteRows.indexOf(n) === -1; }).sort(function (a, b) { return a - b; });
  const keepRowNum = keepCandidates.length ? keepCandidates[0] : matches[0].rowNum;

  result.previewExample = "keep row " + keepRowNum + " -> remove rows " + removeRowsText + " (key " + shortenPreviewKey_(targetKey, 48) + ")";

  if (sortedDeleteRows.length < targetCount) {
    result.warning = "Requested remove=" + targetCount + " but only " + sortedDeleteRows.length + " safe duplicate row(s) matched for key " + shortenPreviewKey_(targetKey, 36) + ".";
  }
...
function buildOutputRowSignature_(rowDisplay) {
  ...
  for (let i = 0; i < coreLen; i++) {
    parts.push(String(row[i] || "").replace(/\s+/g, " ").trim().toLowerCase());
  }
  return parts.join("|");
}
...
function buildDuplicatePreviewExamples_(groups, groupLimit, rowLimit) {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    const rows = g.duplicates.map(function (d) { return Number(d.rowNum); }).filter(function (n) { return !isNaN(n) && n >= 2; }).sort(function (a, b) { return a - b; });
    ...
    examples.push("keep row " + g.keepRowNum + " -> remove rows " + removeRowsText + " (key " + shortenPreviewKey_(key, 48) + ")");
  }
...
function toNonNegativeInt_(value, displayValue) {
  let n = Number(value);
  if (isNaN(n)) n = Number(String(displayValue || "").replace(/,/g, "").replace(/[^0-9.-]/g, ""));
...
function deleteRowsInReverse_(sheet, rows) {
  ...
  const sorted = Object.keys(unique).map(function (k) { return Number(k); }).sort(function (a, b) { return b - a; });
...
function writeCleanupBackupSheet_(ss, records) {
  ...
  const header = ["Source Sheet", "Original Row", "Duplicate Key", "Kept Row", "Row Snapshot"];
  sh.getRange(1, 1, 1, header.length).setValues([header]);

  const values = records.map(function (r) {
    return [r.sourceSheet || "", r.rowNumber || "", r.duplicateKey || "", r.keptRowNumber || "", (r.rowDisplay || []).map(function (c) { return String(c || ""); }).join(" | ")];
  });

  if (values.length > 0) sh.getRange(2, 1, values.length, header.length).setValues(values);
  sh.setFrozenRows(1);
...
function findOutputSheetByYear_(ss, year) {
  ...
  if (mapping && mapping.spreadsheetId) {
    try {
      ...
      if (targetSs) {
        const sheetNameToUse = mapping.sheetName && mapping.sheetName.trim() ? mapping.sheetName : desiredName;
        targetSheet = targetSs.getSheetByName(sheetNameToUse) || targetSs.getSheetByName(desiredName);
        
        if (!targetSheet) {
          targetSheet = targetSs.insertSheet(sheetNameToUse);
          if (targetSheet.getLastRow() === 0) {
            targetSheet.appendRow(COLUMN_MAPPING.concat([CONFIG.sourceHeaderName]).concat(CONFIG.enrichmentHeaders));
          }
        }
      }
    } catch (e) { Logger.log("Error handling mapped spreadsheet for year " + year + ": " + (e && e.message)); }
  }
  ...
  if (!targetSheet) {
    ...
    if (!targetSheet) {
      ...
      } catch (eCreateCtrl) { Logger.log("Cannot create output sheet for year " + year + ": " + (eCreateCtrl && eCreateCtrl.message)); }
    }
  }
