      ...
      if (typeof Drive !== "undefined" && Drive && Drive.Permissions) {
        ...
        for (var pi = 0; pi < items.length; pi++) {
          ...
          if (p.type === "anyone" || p.type === "anyoneWithLink" || (p.type === "domain" && p.role === "reader")) {
            try { Drive.Permissions.remove(folder.getId(), p.id); removed++; } catch (e) { }
          }
        }
        if (removed > 0) Logger.log("Temp folder permissions cleaned: removed " + removed);
      }
    } catch (e) { }

    try {
      var fviewers = folder.getViewers();
      for (var vi = 0; vi < fviewers.length; vi++) { try { folder.removeViewer(fviewers[vi]); } catch (e) { } }
      var feditors = folder.getEditors();
      for (var ei = 0; ei < feditors.length; ei++) { try { folder.removeEditor(feditors[ei]); } catch (e) { } }
      try { folder.setSharing(DriveApp.Access.PRIVATE, DriveApp.Permission.VIEW); } catch (e) { }
    } catch (e) { }
    return folder;
  }
...
function isTempArtifactName_(name) {
  return String(name || "").toUpperCase().indexOf("_TEMP_") === 0;
}
...
function getFileNameSafe_(file) {
  if (!file) return "";
  try { if (typeof file.getName === "function") return String(file.getName() || ""); } catch (e) { }
...
function getFileIdSafe_(file) {
  if (!file) return "";
  try { if (typeof file.getId === "function") return String(file.getId() || ""); } catch (e) { }
  return String(file.id || "");
}

function getFileUrlSafe_(file) {
  if (!file) return "";
  try { if (typeof file.getUrl === "function") return String(file.getUrl() || ""); } catch (e) { }
...
function getFileLastUpdatedSafe_(file) {
  if (!file) return null;
  try { if (typeof file.getLastUpdated === "function") return toValidDate_(file.getLastUpdated()); } catch (e) { }
  return toValidDate_(file.modifiedDate || file.modifiedTime || null);
}

function getFileDateCreatedSafe_(file) {
  if (!file) return null;
  try { if (typeof file.getDateCreated === "function") return toValidDate_(file.getDateCreated()); } catch (e) { }
...
function normalizeDriveApiFileItem_(item) {
  ...
  const url = String(item.alternateLink || item.webViewLink || (id ? "https://drive.google.com/open?id=" + id : ""));
  return { id: id, name: name, url: url, createdDate: toValidDate_(item.createdDate || item.createdTime), modifiedDate: toValidDate_(item.modifiedDate || item.modifiedTime) };
}
...
function detectYearFromNameOrFileDate_(file) {
  ...
  var primaryDate = CONFIG.preferLastUpdatedForYearFallback === false ? getFileDateCreatedSafe_(file) : getFileLastUpdatedSafe_(file);
  var secondaryDate = CONFIG.preferLastUpdatedForYearFallback === false ? getFileLastUpdatedSafe_(file) : getFileDateCreatedSafe_(file);
...
function listSourceFilesIncremental_(sourceFolder, sinceDate) {
  ...
  const pageSize = Math.max(50, Math.min(1000, Number(CONFIG.incrementalScanPageSize) || 250));
  const maxInspect = Math.max(1, Number(CONFIG.incrementalScanMaxFilesToInspect) || 2000);
...
function listSourceFilesForScan_(sourceFolder, scriptProps, options) {
  ...
  const shouldUseFull = forceFullByOption || !useIncremental || runCount % fullEveryRuns === 0;
  ...
  if (!shouldUseFull) {
    mode = "incremental";
    const lookbackHours = Math.max(1, Number(CONFIG.incrementalScanLookbackHours) || 24);
    const overlapMinutes = Math.max(0, Number(CONFIG.incrementalScanOverlapMinutes) || 15);

    sinceDate = toValidDate_(scriptProps.getProperty(lastAtKey));
    if (!sinceDate) sinceDate = new Date(now.getTime() - lookbackHours * 60 * 60 * 1000);
    sinceDate = new Date(sinceDate.getTime() - overlapMinutes * 60 * 1000);

    try {
      files = listSourceFilesIncremental_(sourceFolder, sinceDate);
    } catch (eInc) {
      Logger.log("Incremental scan failed, fallback to full scan: " + (eInc && eInc.message));
      mode = "full-fallback";
  ...
  return { files: files, mode: mode, sinceIso: sinceDate ? sinceDate.toISOString() : "" };
}

function listCandidateFilesByYear_(sourceFiles, processedMap, failedAttemptsMap) {
  const years = getConfiguredYears_();
  const candidates = {};
  years.forEach(function (y) { candidates[y] = []; });
  ...
  for (let idx = 0; idx < files.length && totalNow < maxTotal; idx++) {
    ...
    candidates[year].push({ id: fileId, name: name, year: year, url: fileUrl, dateCreated: getBestFileDateForMetadata_(f) });
    totalNow++;
  }
...
function buildProcessList_(candidates) {
  const years = getConfiguredYears_();
  const perYearLists = years.map(function (y) { return (candidates[y] || []).slice(0, CONFIG.maxFilesPerRunPerYear); });
  const out = [];
  let idx = 0;
...
function convertExcelToTempSheet_(fileInfo, tempFolder) {
  const body = { title: "_TEMP_" + fileInfo.name, mimeType: "application/vnd.google-apps.spreadsheet", parents: [{ id: tempFolder.getId() }] };

  try {
    const copied = Drive.Files.copy(body, fileInfo.id, { convert: true, supportsAllDrives: true });
    try { DriveApp.getFileById(copied.id).moveTo(tempFolder); } catch (e) { }
    try { ensureFileRestricted_(copied.id); } catch (e) { }
    return copied.id;
  } catch (e) {
    const errMsg = ((e && e.message) || "").toLowerCase();
    if (errMsg.indexOf("request too large") !== -1 || errMsg.indexOf("413") !== -1) {
      throw new Error("Needs manual check - file too large");
    }

    try {
      const copiedFallback = Drive.Files.copy(body, fileInfo.id, { convert: true });
      try { DriveApp.getFileById(copiedFallback.id).moveTo(tempFolder); } catch (e) { }
      try { ensureFileRestricted_(copiedFallback.id); } catch (e) { }
      return copiedFallback.id;
    } catch (e2) {
      const errMsg2 = ((e2 && e2.message) || "").toLowerCase();
      if (errMsg2.indexOf("request too large") !== -1 || errMsg2.indexOf("413") !== -1) throw new Error("NON_RETRIABLE_TOO_LARGE: Request Too Large");
      if (errMsg2.indexOf("internal error") !== -1 || (errMsg2.indexOf("drive.files.copy") !== -1 && errMsg2.indexOf("failed") !== -1)) throw new Error("Needs manual check - unsupported format");
      throw e2;
    }
  }
}
...
function ensureFileRestricted_(fileId) {
  ...
  try {
    if (typeof Drive !== "undefined" && Drive && Drive.Permissions) {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        if (!p || !p.type) continue;
        if (p.type === "anyone" || p.type === "anyoneWithLink" || (p.type === "domain" && p.role === "reader")) {
          try { Drive.Permissions.remove(fileId, p.id); removed++; } catch (e) { }
        }
      }
      if (removed > 0) Logger.log("Temp file permissions cleaned: removed " + removed);
    }
  } catch (e) { }
  ...
  try {
    ...
    var viewers = [];
    var editors = [];
    try { viewers = f.getViewers(); editors = f.getEditors(); } catch (e) { }
    for (var vi = 0; vi < viewers.length; vi++) { try { f.removeViewer(viewers[vi]); } catch (e) { } }
    for (var ei = 0; ei < editors.length; ei++) { try { f.removeEditor(editors[ei]); } catch (e) { } }
    try { f.setSharing(DriveApp.Access.PRIVATE, DriveApp.Permission.VIEW); } catch (e) { }
  } catch (e) { }
}
...
function openSpreadsheetWithRetry_(fileId, attempts, delayMs) {
  ...
  for (let i = 1; i <= attempts; i++) {
    try { return SpreadsheetApp.openById(fileId); } catch (e) {
      lastErr = e;
...
function parseConvertedSheet_(tempSS, fileName) {
  ...
  const directNames = ["GR TEMPLATE", "GR Template", "GR template", "GRTEMPLATE", "GR TEMPLATE "];
  for (let i = 0; i < directNames.length; i++) {
    ...
    } catch (e) { Logger.log("Direct-name check failed for " + directNames[i] + " (" + e.message + ")"); }
  }

  if (CONFIG.preferDirectTemplateTabFastPath) {
    const fastDirect = pickBestCandidate_(directPreferredCandidates) || pickBestCandidate_(directFallbackCandidates);
    if (fastDirect && fastDirect.rows && fastDirect.rows.length > 0) {
      Logger.log("Fast-path selected '" + fastDirect.sheetName + "' with " + fastDirect.rows.length + " row(s).");
  ...
  try {
    ...
    for (let i = 0; i < sheets.length; i++) {
      let sh = sheets[i];
      let nm = "";
      try { nm = sh.getName(); } catch (e) { Logger.log("Skipping unnamed/problem sheet (" + e.message + ")"); continue; }
      if (seenNames[nm]) continue;
      seenNames[nm] = true;
      let candidate = null;
      try { candidate = evaluateSheetCandidate_(sh, fileName, false); } catch (e) { Logger.log('Skipping sheet "' + nm + '" in parse loop (' + e.message + ")"); continue; }
      if (!candidate) continue;
      if (candidate.preferred) preferredCandidates.push(candidate);
      else fallbackCandidates.push(candidate);
    }
  } catch (e) { Logger.log("Could not enumerate all sheets for " + fileName + " (" + e.message + ")"); }

  const bestDirect = pickBestCandidate_(directPreferredCandidates) || pickBestCandidate_(directFallbackCandidates);
  if (bestDirect && bestDirect.rows && bestDirect.rows.length > 0) {
    Logger.log("Selected '" + bestDirect.sheetName + "' with " + bestDirect.rows.length + " row(s).");
...
function evaluateSheetCandidate_(sh, fileName, isDirectName) {
  try {
    ...
    try {
      ...
      if (CONFIG.useNumberFormatCurrencyHints) fmts = sh.getRange(1, 1, lastRow, lastCol).getNumberFormats();
    } catch (e) { Logger.log('Skipping sheet "' + sh.getName() + '" (' + e.message + ")"); return null; }
    ...
    const keyHeaderCount = countMappedHeaders_(h.columnMap, ["PO No.", "Material Description", "GR Mat. Doc.", "Amount To Billed", "Payment Milestone"]);
    ...
    let rows = extractRowsWithFilter_(vals, disp, fmts, h.columnMap, h.headerRowIndex, true, sh);
    if (rows.length === 0) rows = extractRowsWithFilter_(vals, disp, fmts, h.columnMap, h.headerRowIndex, false, sh);
    if (rows.length === 0) rows = extractRowsByAnchors_(vals, disp, fmts, h.columnMap, h.headerRowIndex, sh);
    ...
    return {
      ...
      score: h.matchedCount * 100 + Math.min(rows.length, 50) + (isDirectName ? 1000 : 0) + computePoHintMatchScore_(rows, fileName),
    };
  } catch (e) {
    Logger.log('Skipping sheet "' + sh.getName() + '" due to error: ' + e.message);
    return null;
  }
}
...
function extractRowsWithFilter_(values, display, formats, columnMap, headerRowIndex, strictMode, sheet) {
  const rows = [];
  const anchors = ["PO No.", "Material Description", "Amount To Billed", "Payment Milestone", "GR Mat. Doc."];

  for (let r = headerRowIndex + 1; r < values.length; r++) {
    if (CONFIG.onlyIncludeVisibleRows && !isSheetRowVisible_(sheet, r + 1)) continue;
...
function extractRowsByAnchors_(values, display, formats, columnMap, headerRowIndex, sheet) {
  const rows = [];
  const anchors = ["PO No.", "Material Description", "Amount To Billed", "Payment Milestone", "GR Mat. Doc."];
  ...
  for (let r = headerRowIndex + 1; r < values.length; r++) {
    ...
    for (let i = 0; i < anchors.length; i++) {
      const idx = columnMap[anchors[i]];
      if (idx === undefined) continue;
      if (isCellPresent_(rawRow[idx], dispRow[idx])) { hasAnchor = true; break; }
    }
...
function isLikelyGRTemplateSheet_(sheetName) {
  ...
  return name === "gr template" || name.indexOf("gr template") !== -1 || name.indexOf("gr_template") !== -1 || name.indexOf("gr-template") !== -1 || name.indexOf("grtemplate") !== -1;
}
...
function createColumnMapping_(headerRow) {
  ...
  COLUMN_MAPPING.forEach(function (canonical) {
    ...
    const compactAliases = aliases.map(function (a) { return compact(a); });
    ...
    for (let c = 0; c < headerRow.length; c++) {
      ...
      for (let a = 0; a < normAliases.length; a++) {
        ...
        const normMatch = cellNorm === aliasNorm || (cellNorm.length >= 5 && aliasNorm.length >= 5 && (cellNorm.indexOf(aliasNorm) !== -1 || aliasNorm.indexOf(cellNorm) !== -1));
        const compactMatch = cellCompact === aliasCompact || (cellCompact.length >= 5 && aliasCompact.length >= 5 && (cellCompact.indexOf(aliasCompact) !== -1 || aliasCompact.indexOf(cellCompact) !== -1));
...
function isLikelyMergedArtifactRow_(rawRow, dispRow) {
  ...
  for (let i = 0; i < rawRow.length; i++) {
    const v = String(disp[i] !== undefined && disp[i] !== "" ? disp[i] : rawRow[i] || "").replace(/\u00a0/g, " ").trim();
    if (!v) continue;
...
function isSheetRowVisible_(sheet, rowNumber) {
  ...
  try { if (typeof sheet.isRowHiddenByFilter === "function") { if (sheet.isRowHiddenByFilter(rowNumber)) return false; } } catch (e) { }
  try { if (typeof sheet.isRowHiddenByUser === "function") { if (sheet.isRowHiddenByUser(rowNumber)) return false; } } catch (e) { }
  return true;
}
