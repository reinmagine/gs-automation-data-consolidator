...
function consolidateGRTemplateData() {
  ...
  try {
    ...
    if (!ss) {
      ...
      scriptProps.setProperty("LAST_RUN_STATUS", "Stopped: spreadsheet not found (run setup first)");
      scriptProps.setProperty("LAST_RUN_FINISHED_AT", new Date().toString());
      return;
    }
    ...
    if (!sourceFolder) {
      ...
      scriptProps.setProperty("LAST_RUN_STATUS", "Stopped: source folder not found");
      scriptProps.setProperty("LAST_RUN_FINISHED_AT", new Date().toString());
      return;
    }
    ...
    scriptProps.setProperty("LAST_RUN_SCAN_MODE", scanState.mode + (scanState.sinceIso ? " (since " + scanState.sinceIso + ")" : ""));

    const candidates = listCandidateFilesByYear_(scanState.files, processedMap, failedAttemptsMap);
    const years = getConfiguredYears_();
    const yearCounts = years.map(function (y) { return y + "=" + ((candidates[y] && candidates[y].length) || 0); }).join(", ");
    ...
    for (let i = 0; i < toProcess.length; i++) {
      if (Date.now() - startTime > CONFIG.maxRuntimeMs) {
        ...
        scriptProps.setProperty("LAST_RUN_STATUS", "Stopped early: runtime limit reached");
      ...
      if (hasAnyProcessingKeyMatch_(doneKeyMap, fileInfo)) {
        ...
        Logger.log("Skipping file already present in tracker log: " + fileInfo.name);
        continue;
      }
    ...
    scriptProps.setProperty("LAST_RUN_STATUS", "Done: files=" + processedCount + ", rows=" + totalRowsAdded);
    scriptProps.setProperty("LAST_RUN_STAGE", "Idle");
    scriptProps.setProperty("LAST_RUN_ACTIVE_FILE", "");
    scriptProps.setProperty("LAST_RUN_ACTIVE_YEAR", "");
    scriptProps.setProperty("LAST_RUN_FINISHED_AT", new Date().toString());
...
function runConsolidateForYear(year) {
  ...
  try {
    ...
    const scanState = listSourceFilesForScan_(sourceFolder, scriptProps, { forceFull: true });
    const candidates = listCandidateFilesByYear_(scanState.files, processedMap, failedAttemptsMap);
    ...
    return "Done: files=" + processedCount + ", rows=" + totalRowsAdded + " (scan mode: " + scanState.mode + ")";
  } catch (e) {
...
function processSingleFile_(ss, fileInfo, tempFolder) {
  ...
  try {
    ...
    if (!outputSheet) {
      ...
      try {
        outputSheet = ss.getSheetByName(outputSheetName) || ss.insertSheet(outputSheetName);
        ...
        Logger.log("Could not create output sheet for year " + fileInfo.year + " (" + (eCreate && eCreate.message) + ")");
        throw eCreate;
      }
    }

    if (outputSheetHasSourceFile_(outputSheet, fileInfo)) {
      Logger.log("Skipping append; source already exists in output: " + fileInfo.name);
      timings.totalMs = Date.now() - t0;
      timings.rowsAdded = 0;
    ...
    if (msgLower.indexOf("needs manual check - file too large") !== -1 || msgLower.indexOf("request too large") !== -1 || msgLower.indexOf("413") !== -1) {
      timings.status = "Needs manual check - file too large";
      return { rowsAdded: 0, status: timings.status, timings: timings };
    }

    if (msgLower.indexOf("needs manual check - unsupported format") !== -1 || msgLower.indexOf("conversion of the uploaded content") !== -1 || (msgLower.indexOf("drive.files.copy") !== -1 && msgLower.indexOf("internal error") !== -1)) {
      return { rowsAdded: 0, status: "Needs manual check - unsupported format", timings: timings };
    }
...
function debugAutoProcessingStatus() {
  ...
  if (sourceFolder) {
    ...
    while (files.hasNext()) {
      ...
      const isExcel = lower.indexOf(".xlsx") !== -1 || lower.indexOf(".xls") !== -1;
      if (!isExcel) continue;
  ...
  const msg = [
    ...
    "Last scan mode: " + (scriptProps.getProperty("LAST_RUN_SCAN_MODE") || scriptProps.getProperty("SOURCE_SCAN_LAST_MODE") || "N/A"),
    "Last scanned file list size: " + (scriptProps.getProperty("SOURCE_SCAN_LAST_FILE_COUNT") || "N/A"),
    "Last incremental since: " + (scriptProps.getProperty("SOURCE_SCAN_LAST_SINCE") || "N/A"),
    "",
    "Source folder stats",
    "Excel files total: " + totalExcel,
    "Excel files mapped to " + getConfiguredYears_().join("/") + " (name/date fallback): " + withYear,
  ...
  if (sampleNoYear.length) {
    msg.push("");
    msg.push("Sample skipped files (cannot map to " + getConfiguredYears_().join("/") + " via name/date):");
    sampleNoYear.forEach(function (n) { msg.push("- " + n); });
  }
...
function debugTestSingleFile() {
  ...
  while (files.hasNext()) {
    ...
    const hasYear = years.some(function (y) { return n.indexOf(y) !== -1; });
  ...
  if (!testFile) {
    SpreadsheetApp.getUi().alert("No Excel file found for configured years: " + getConfiguredYears_().join(", "));
    return;
  }
...
function debugMainSiteSetup() {
  ...
  const allNames = ss.getSheets().map(function (s) { return s.getName(); });
  let msg = [];
